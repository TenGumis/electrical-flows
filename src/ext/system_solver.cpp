...
std::vector<double> SystemSolver::solve(const Matrix &matrix) {
    // capture time on start
    auto start = std::chrono::high_resolution_clock::now();

    std::cerr << "Solving matrix:" << std::endl << matrix;

    Matrix matrix_U = decompose_lu(matrix).second;
    matrix_U.augmented = true;

    std::cerr << "Upper triangular matrix:" << std::endl << matrix_U;
    ...
    std::cerr << "Pivots are on columns:" << std::endl << pivots_column_indexes;
    std::cerr << "Pivots are on rows:" << std::endl << pivots_row_indexes;
    ...
    if (!solution_exists) {
        std::cerr << "Given linear system has no solution." << std::endl;
    } else {
        std::cerr << "Solution of the linear system:" << std::endl;
        ...
        // print the solution
        if (!kernel.empty()) {
            std::cerr << "Particular solution is:" << std::endl;
            std::cerr << vector_p;
            std::cerr << "Basis of the kernel is:" << std::endl;
            for (const auto &i : kernel) {
                std::cerr << i;
            }
        } else {
            std::cerr << "Unique solution is:" << std::endl;
            std::cerr << vector_p;
        }
    }
    ...
    std::cerr << "Needed " << to_ms(end - start).count() << " ms to finish." << std::endl;
