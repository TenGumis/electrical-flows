                                             ...
                                             const int flow)
{
  ...
  for (int i = 0; i < edges.size(); i++)
  {
    ...
    auto newEdge = std::make_shared<MatchingEdge>(matchingNodeP.get(), matchingNodeQ.get(), edges[i]->flow);
  ...
  for (int i = 0; i < nodes.size(); i++)
  {
    ...
    for (int j = 0; j < nodes[i]->outgoingEdges.size(); j++)
    {
      ...
      auto newEdge = std::make_shared<MatchingEdge>(
              outgoingEdge->matchingNodeP, matchingNodeQ.get(), outgoingEdge->capacity - outgoingEdge->flow);
    ...
    for (int j = 0; j < nodes[i]->incomingEdges.size(); j++)
    {
      ...
      auto newEdge = std::make_shared<MatchingEdge>(
              matchingNodeP.get(), incomingEdge->matchingNodeQ, incomingEdge->capacity - incomingEdge->flow);
  ...
  for (int j = 0; j < nodes[t]->incomingEdges.size(); j++)
  {
    ...
    auto newEdge = std::make_shared<MatchingEdge>(
            matchingNodePT.get(), incomingEdge->matchingNodeQ, incomingEdge->capacity - incomingEdge->flow);
  ...
  for (int j = 0; j < nodes[s]->outgoingEdges.size(); j++)
  {
    ...
    auto newEdge = std::make_shared<MatchingEdge>(
            outgoingEdge->matchingNodeP, matchingNodeQS.get(), outgoingEdge->capacity - outgoingEdge->flow);
